...
- 新增通用图片跳转弹窗处理：关闭通过 `sports.qq.com/sapp/h5msg.htm` 打开的全屏图片运营弹窗，例如“仙逆战天道”等游戏推广。
- 优化脚本职责划分：
  - `TencentSportsAdBlock.js` 仅处理首页信息流、赛事页、文章页和首页会员推广等 feeds/list 相关内容；
  - `TencentSportsFloatBlock.js` 仅处理比赛页横幅、首页悬浮入口及独立整屏弹窗。
- 避免两个脚本同时修改同一个接口响应，降低规则互相覆盖、部分广告重新出现的风险。
- 保留正常首页赛事日程、快讯模块、比赛直播卡及视频播放相关接口。
